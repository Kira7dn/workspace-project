...
const SingleProject = ({ project, action }) => {
  const {
    title,
    deadline,
    leader,
  ...
  return (
    <>
      <div
        ...
        onClick={action.bind(this, project)}
      >
        <div className="p-2">
          <div className="mb-4">
            <Row>
              <Col
                className={`${styles.projectlistitemtitle} col-6 p-0`}
              >
                <img
                  src={projectIcon}
                  alt="project"
                ...
                />
                <p
                  className={`${styles.projectlistitemcontent} text-primary`}
                >
                  {title}
                </p>
              </Col>
            ...
            <Row>
              <Col className="col-4">
                <div className={styles.projectlistitemtarget}>
                  <h5 className={styles.projectlistitemtargettitle}>
                    Target
                  </h5>
              ...
              <Col className="col-2 p-0">
                <Row className={`${styles.leaderContainer} mx-0`}>
                  <h5 className={styles.leaderTitle}>
                    {`Team (${
                      members ? members.length : 0 + leader ? 1 : 0
                    })`}
                  </h5>
                </Row>
                    ...
                    ? members.map((member) => {
                        return (
                          <div
                            key={member.id}
                            className={
                              styles.projectlistitemmemberinfo
                            }
                          >
                            <img
                              src={member.avatar}
                              alt={member.username}
                              className={
                                styles.projectlistitemmemberavatar
                              }
                            />
                            <div
                ...
                <div className={styles.projectlistitemactionitemlist}>
                  ...
                  >
                    {children.map((child) => (
                      ...
                      >
                        <div
                          className={`col-6 ${styles.ActionItemTitle}`}
                        >
                          {child.title}
                        </div>
                        <div
                        ...
                        >
                          <ProgressBar
                            className={
                              styles.projectlistitemchildprogressbar
                            }
                            animated
                            variant="success"
                            now={child.progress}
